export function startTriPong() {
    ...
    const paddleAngles = [Math.PI / 2, -Math.PI / 6, 7 * Math.PI / 6];
    ...
    window.onkeydown = e => {
        if (e.key === 'z')
            paddleStates[0].offset = -10;
        if (e.key === 's')
            paddleStates[0].offset = +10;
        if (e.key === 'i')
            paddleStates[1].offset = -10;
        if (e.key === 'k')
            paddleStates[1].offset = +10;
        if (e.key === 'ArrowLeft')
            paddleStates[2].offset = -10;
        if (e.key === 'ArrowRight')
            paddleStates[2].offset = +10;
    };
    window.onkeyup = e => {
        if (['z', 's'].includes(e.key))
            paddleStates[0].offset = 0;
        if (['i', 'k'].includes(e.key))
            paddleStates[1].offset = 0;
        if (['ArrowLeft', 'ArrowRight'].includes(e.key))
            paddleStates[2].offset = 0;
    };
}
