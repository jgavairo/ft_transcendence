...
function displayWinScreen(winner: string): void {
  const canvas = document.getElementById('pongCanvas') as HTMLCanvasElement;
  if (!canvas) return;
  const ctx = canvas.getContext('2d')!;
  canvas.width = canvas.clientWidth;
...
export function startPong(): void {
  ...
  function updatePaddles(): void {
    if (keys['w'] || keys['W']) leftPaddleY -= paddleSpeed;
    if (keys['s'] || keys['S']) leftPaddleY += paddleSpeed;
  ...
  function drawLivesWrapper(): void {
