...
export async function googleSignInHandler() {
    ...
    const popup = window.open(
        `https://${HOSTNAME}:8443/api/auth/google`,
        'googleAuth',
        `width=${popupWidth},height=${popupHeight},left=${left},top=${top},scrollbars=yes,resizable=yes,menubar=no,toolbar=no,location=no,status=no`
    );
