 ...
 */
export async function renderLibrary(query: string): Promise<void> {
  ...
  const listEl = document.querySelector('.library-games-list') as HTMLElement;
  ...
  function highlightSidebar(id: number) {
    ...
    if (lineEl) lineEl.classList.add('activegamesidelist');
  }
