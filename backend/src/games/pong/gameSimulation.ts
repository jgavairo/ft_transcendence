...
export interface MatchState {
  ...
  const CANVAS_HEIGHT = 800;   // Exemple : hauteur du canvas
  const BALL_RADIUS = 10;      // Rayon de la balle
  const INITIAL_BALL_SPEED_X = 6;
  const INITIAL_BALL_SPEED_Y = 3;
  const PADDLE_WIDTH = 10;
  const PADDLE_HEIGHT = 100;
  const PADDLE_SPEED = 5;      // Vitesse de déplacement des paddles
  const INITIAL_LIVES = 5;     // Nombre de vies initiales
  ...
  export function startMatch(
    ...
    const intervalId = setInterval(() => {
      if (matchState.gameOver) {
        ...
        io.to(roomId).emit('gameOver', { winner: (matchState.leftLives <= 0 ? player2.username : player1.username) });
        return;
      }
  ...
  function updateGameState(matchState: MatchState): void {
    // Mettre à jour les positions des paddles selon leur direction
